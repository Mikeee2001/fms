...
export default function ProductForm({ product = null, categories = [], customizationCategories = [], selectedCustomizations = {} }) {
    ...
    //Check image if jpg, png, jpeg and gif
    //if not it display error message
    const validateImage = (file) => {
        ...
        if (file.size > maxSize) {
            return `${file.name}: Image must be less than 2MB.`;
        }
                ...
                <div className="flex flex-wrap gap-2">
                    {tabs.map(tab => (
                        <button
                            ...
                            className={`flex items-center px-4 py-2 text-sm font-medium rounded-t-lg transition-colors ${activeTab === tab.id
