...
class ReportController extends Controller
{
    ...
    public function salesReport()
    {
        ...
        // GROUP BY ISO WEEK (stable grouping)
        $ordersByWeek = $orders->groupBy(function ($order) {
            return $order->created_at->format('o-\WW'); // e.g. 2026-W28
        });

        $weeklyData = [];

        foreach ($ordersByWeek as $weekKey => $orders) {

            $firstOrderDate = $orders->first()->created_at;

            $weekStart = $firstOrderDate->copy()->startOfWeek(Carbon::MONDAY);
            $weekEnd = $firstOrderDate->copy()->endOfWeek(Carbon::SUNDAY);

            $weekLabel = $weekStart->format('M d') . ' - ' . $weekEnd->format('M d, Y');

            $subtotalSales = $orders->sum('total_price');

            $weekData = [
                'week' => $weekLabel,
                'total_sales' => $subtotalSales,
            ...
            // GRAND TOTALS
            $grandTotals['total_sales'] += $weekData['total_sales'];
            $grandTotals['total_orders'] += $weekData['order_count'];
            $grandTotals['cod_count'] += $weekData['payment_methods']['cod'];
            $grandTotals['gcash_count'] += $weekData['payment_methods']['gcash'];
            $grandTotals['paymaya_count'] += $weekData['payment_methods']['paymaya'];
            $grandTotals['bank_transfer_count'] += $weekData['payment_methods']['bank_transfer'];
        }
